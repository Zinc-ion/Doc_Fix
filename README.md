...
- 将第一部分到第五部分内的表格文字统一为宋体/Times New Roman 五号；范围外表格不做表格文字格式修正。
- 将第一部分到第五部分内已有表格边框线宽统一为非加粗；不新增缺失边框，不修改表格结构。
- 将第一部分到第五部分内图片段落居中，并清除图片左侧前置空白；不修改图片大小、裁剪、环绕方式或锚点。
- 将第一部分到第五部分内已有图题/表题转换为 Word `SEQ` 题注，图和表分别全局连续编号，宋体/Times New Roman 五号居中；缺失标题不自动新增。
- 不自动调整图片大小、表格宽度、表格结构、页眉页脚和页面设置。
