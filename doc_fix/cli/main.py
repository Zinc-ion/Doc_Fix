...
from doc_fix.config import apply_rule_config, load_rule_config
...
from doc_fix.reporter import render_report, write_html_report, write_json_report, write_markdown_report
...
def check(
    ...
    if config_path is not None:
        try:
            rules = apply_rule_config(rules, load_rule_config(config_path))
        except ValueError as exc:
            raise click.ClickException(str(exc)) from exc
    issues = checker.check(template_snapshot, input_snapshot, rules)
    report = CheckReport(
    ...
    if enable_ai:
        report = _with_ai_assistance(report)
...
def _with_ai_assistance(report: CheckReport) -> CheckReport:
    try:
        assistance = DeepSeekAssistant().analyze(report)
