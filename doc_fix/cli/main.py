...
from doc_fix.service import run_correction
...
def correct(
    ...
    converter = WordConverter()
    try:
        artifacts = run_correction(template_path, input_path, out_dir, converter, config_path=config_path)
    except (ConversionError, ValueError) as exc:
        raise click.ClickException(str(exc)) from exc

    click.echo(f"Corrected DOCX: {artifacts.corrected_docx_path}")
    if artifacts.correction_report.corrected_doc_path:
        click.echo(f"Corrected DOC: {artifacts.correction_report.corrected_doc_path}")
    if artifacts.correction_report.corrected_doc_export_error:
        click.echo(f"DOC export skipped/failed: {artifacts.correction_report.corrected_doc_export_error}")
